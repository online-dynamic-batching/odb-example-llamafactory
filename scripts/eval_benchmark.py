#!/usr/bin/env python3
"""Evaluate a saved MM-Mix checkpoint on MMMU-MC.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=os.getenv("ODB_LF_EVAL_CHECKPOINT"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=env_first("ODB_LF_BENCHMARK_OUTPUT_DIR", "ODB_LF_EVAL_SAVE_DIR"),
    )
    parser.add_argument(
        "--dataset",
        default=os.getenv("ODB_LF_BENCHMARK_DATASET", "MMMU/MMMU"),
    )
    parser.add_argument(
        "--split",
        default=os.getenv("ODB_LF_BENCHMARK_SPLIT", "validation"),
    )
    parser.add_argument("--subjects", default=os.getenv("ODB_LF_EVAL_SUBJECTS"))
    parser.add_argument(
        "--max-samples",
        type=int,
        default=int(os.getenv("ODB_LF_EVAL_MAX_SAMPLES", "0")),
    )
    parser.add_argument(
        "--torch-dtype",
        default=os.getenv("ODB_LF_EVAL_TORCH_DTYPE", "bf16"),
...
def main() -> None:
    ...
    if args.checkpoint is None:
        raise SystemExit("--checkpoint or ODB_LF_EVAL_CHECKPOINT is required")
    checkpoint = args.checkpoint.resolve()
    if not checkpoint.exists():
        raise FileNotFoundError(f"checkpoint not found: {checkpoint}")

    output_dir = args.output_dir or (checkpoint / "mmmu_mc_likelihood_public_lf")
