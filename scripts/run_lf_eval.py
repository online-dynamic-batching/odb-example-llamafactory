...
def _write_eval_config(
    *, checkpoint: Path, train_config: Path, save_dir: Path, val_size: float | None
) -> Path:
    cfg = _load_yaml(train_config)
    effective_val_size = val_size
    if effective_val_size is None:
        effective_val_size = float(cfg.get("val_size", 0.05))
    for key in list(cfg):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Validation split ratio used by LLaMA-Factory valloss eval. Defaults to the training config val_size.",
    )
    parser.add_argument(
