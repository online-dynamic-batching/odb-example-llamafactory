    ...
    hook_note.write_text(
        "\n".join(
            [
                ...
                "```",
                "",
                f"`patches/{ENABLE_ODB_PATCH.name}` is only a developer reference for",
                "already TMDB/lazy-capable LLaMA-Factory forks.",
                "",
        ...
        path.write_text(
            "\n".join(
                [
                    ...
                    "            'compatible checkout, or set LLAMAFACTORY_ROOT to an equivalent fork that calls '",
                    "            'odb.integrations.llamafactory.enable_odb(...) after Trainer/DataLoader construction. '",
                    "            f'Developer patch reference: {ENABLE_ODB_PATCH}'",
                    "        )",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image-max-pixels", type=int, default=9437184)
